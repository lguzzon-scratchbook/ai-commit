...
installIfNotPresent "binutils"
installIfNotPresent "build-essential"

export USE_GCC=${1-}
if [ -z "$USE_GCC" ] && [ "${USE_GCC+xyz}" = "xyz" ]; then
  USE_GCC=$(apt-cache search "gcc-" | grep "^gcc-[0-9\.]*-.*$" | sed -n "s/^gcc-\([0-9\.]*\)-.*$/\1/p" | sort -n | tail -1)
fi
