...
mustBeHere curl
readonly lOutputPath=${1:-${current_dir}/portainerBash/}
rm -Rf "${lOutputPath}"
mkdir -p "${lOutputPath}"
curl -sSL "https://api.swaggerhub.com/apis/deviantony/Portainer/1.23.0/swagger.yaml" -o "${lOutputPath}/swagger.yaml"
"${script_dir}/openApiGenerator.sh" generate -i "${lOutputPath}/swagger.yaml" -g bash -o "$lOutputPath"
chmod +x "${lOutputPath}/client.sh"
"${lOutputPath}/client.sh" --version
"${lOutputPath}/client.sh" --help
# Script End
