...
#Required
readonly current_dir_name=$(basename "${current_dir}")

[[ -f "${HOME}/.ssh/id_rsa.pub" ]] || (mkdir -p "${HOME}/.ssh" && chmod 700 "${HOME}/.ssh" && sudo apt -y install openssh-client && ssh-keygen -t rsa -f "${HOME}/.ssh/id_rsa" -P "")
echoExecOk ssh-copy-id -i "${HOME}/.ssh/id_rsa.pub" "root@$1"
echoExecOk ssh -i "${HOME}/.ssh/id_rsa" "root@$1" "rm -rf ~/${current_dir_name}"
echoExecOk scp -i "${HOME}/.ssh/id_rsa" -r "${current_dir}" "root@$1:~/${current_dir_name}"
