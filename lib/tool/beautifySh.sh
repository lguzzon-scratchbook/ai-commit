...
formatFiles() {
  for f in $1; do
    echoExecOk shfmt -w -bn -ci -i 2 -s "$f"
  done
}

(which shfmt >/dev/null || "${script_dir}/toolManager.sh" -shfmt_i)
lFiles="${1-}"
if (isEmpty "${lFiles}"); then
  lFiles=$(shfmt -w -bn -ci -i 2 -f -s "${current_dir}")
elif [[ -d ${lFiles} ]]; then
  lFiles=$(shfmt -w -bn -ci -i 2 -f -s "${lFiles}")
fi
echoExecOk formatFiles "${lFiles}"
# Script End
