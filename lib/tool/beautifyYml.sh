...
formatFile() {
  if (isEmpty "${lSorted}"); then
    yq eval "$1" -P -I2 >yq.tmp
  else
    yq eval 'sortKeys(..)' "$1" -P -I2 >yq.tmp
  fi
  mv yq.tmp "$1"
}
...
formatFileOK() {
  echo "$1"
  echoExecOk formatFile "$1"
}

formatDir() {
  for lYml in $(find "$1" -iname '*.yml' -exec echo {} \;); do
    formatFileOK "$lYml"
  done
  for lYaml in $(find "$1" -iname '*.yaml' -exec echo {} \;); do
    formatFileOK "$lYaml"
  done
}

main() {
  (which shfmt >/dev/null || "${script_dir}/toolManager.sh" -yq_i)
  if (isEmpty "${lFiles}"); then
    formatDir "."
  elif [[ -d $lFiles ]]; then
    formatDir "$lFiles"
  elif [[ -f $lFiles ]]; then
