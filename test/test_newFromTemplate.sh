...
# Script Begin
function main() {
  local -r lNewFromTemplateFile="${script_dir}/$(getUUID)"
  echoExecOk "${script_dir}/../template/newFromTemplate.sh" "${lNewFromTemplateFile}.sh"
  echoExecOk "${lNewFromTemplateFile}.sh"
  echoExecOk rm "${lNewFromTemplateFile}.sh.old"
  echoExecOk rm "${lNewFromTemplateFile}.sh"
}
