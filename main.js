...
async function commitRelease () {
  ...
  const prompt = `Craft a concise yet comprehensive release summary that begins with an imperative verb and remains under 80 characters. This summary should encapsulate the essence of all [Git commits] since the previous release. Consider the following aspects: What are the most significant changes or features introduced? Are there any recurring themes or areas of focus in the commits? How do these changes impact the overall functionality or user experience? Are there any critical bug fixes or performance improvements? Does the commit history reveal any shifts in development priorities or approaches? Additionally, reflect on how this summary can effectively communicate the release's value to both technical and non-technical stakeholders. How can you balance technical accuracy with accessibility in your language choice? After creating your summary, analyze its effectiveness. Does it capture the key developments while adhering to the character limit? How might you refine it further to maximize its impact within the given constraints?:\n[Git commits]\n${commitsText}`
